...
try {
    winget upgrade --all --silent --include-unknown --force --accept-source-agreements --accept-package-agreements -e
    Write-Host "All updates completed successfully. Or not... ^^ See above for more details ^^" -ForegroundColor Green
} catch {
    Write-Host "An error occurred during the update process: $($_.Exception.Message)" -ForegroundColor Red
}

# Prompt user with closing message
